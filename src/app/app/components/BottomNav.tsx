...
import { Icon, type AnIconName } from './Icon';
import { useTranslation } from '@/lib/i18n';

export type AnTab = 'overview' | 'sales' | 'events' | 'settings';

export function BottomNav({ active, onSelect }: { active: AnTab; onSelect: (t: AnTab) => void }) {
  const { t } = useTranslation();
  const ITEMS: { key: AnTab; icon: AnIconName; label: string }[] = [
    { key: 'overview', icon: 'chart',    label: t.analytics.nav.overview },
    { key: 'sales',    icon: 'coins',    label: t.analytics.nav.sales    },
    { key: 'events',   icon: 'bell',     label: t.analytics.nav.events   },
    { key: 'settings', icon: 'settings', label: t.analytics.nav.settings },
  ];
  return (
    <nav style={{
