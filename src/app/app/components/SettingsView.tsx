...
// A proper app Settings page for Analytics (modeled on tec-assets' settings):
...
export function SettingsView({ isAdmin, onLogout }: { isAdmin: boolean; onLogout: () => void }) {
  const { t, locale, setLocale } = useTranslation();
  const { user, isAuthenticated } = usePiAuth();
  const me = useMe();
  const s = t.analytics.settings;
  ...
  const plan = isPro ? s.planPro : s.planFree;
  ...
  return (
    <div>
      ...
      <Section title={s.about} icon="ℹ️">
        ...
        <Row label={s.domain}><span style={{ color: TEC_COLORS.subtext, fontSize: 14 }}>analytics.pi</span></Row>
